...
function ecgY(rawPhase: number): number {
  ...
  if (p < 0.17)  return 0;
  if (p < 0.195) return -0.09; // Q
  if (p < 0.215) return -1.0;  // R spike
  if (p < 0.235) return 0.22;  // S
  if (p < 0.28)  return 0;
...
export const ECGLine: React.FC = () => {
  ...
  const opacity =
    interpolate(p2, [0, 0.18, 1], [0, 0.82, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) *
    interpolate(p3, [0, 0.3, 0.85, 1], [1, 0.8, 0.2, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
    ...
    <AbsoluteFill style={{ pointerEvents: "none", opacity }}>
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} style={{ position: "absolute", inset: 0 }}>
        ...
        <g transform={`translate(${cx - W / 2},${cy - H / 2})`} mask="url(#ecgFadeMask)">
          <polyline points={pStr} fill="none" stroke="rgba(255,60,40,0.52)" strokeWidth={6} style={{ filter: "blur(6px)" }} />
          <polyline points={pStr} fill="none" stroke="rgba(255,145,90,0.94)" strokeWidth={1.9} strokeLinejoin="round" />
        </g>
        <circle cx={cx + W / 2} cy={cursorY} r={3.5} fill="rgba(255,185,100,1)" opacity={opacity} />
        <circle cx={cx + W / 2} cy={cursorY} r={7}   fill="rgba(255,100,55,0.36)" opacity={opacity} style={{ filter: "blur(3px)" }} />
      </svg>
    </AbsoluteFill>
  );
};
